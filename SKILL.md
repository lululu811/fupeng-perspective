...
公开语料只能重建「结论」，不能重建「过程」。因此：

1. **过程 > 结论**：同等条件下，优先采集"被追问为什么"的素材
2. **矛盾保留**：内在张力是人格的核心特征，不和稀泥
3. **时间感知**：思想有演化，Skill 应该有"时效性"标注
4. **承认局限**：通过置信度标注让用户判断可信度
...
| Agent | 搜索目标 | 提取重点 | 输出文件 | 优先级 |
|-------|---------|---------|---------|--------|
| 1 著作 | 书、长文、论文、newsletter | 反复出现的核心论点（≥3次=真信念）、自创术语、推荐书单 | `01-writings.md` | ★★★★ |
| 2 对话 | 播客、长视频、AMA、深度采访 | 被追问时的回答方式、即兴类比、改变立场的瞬间、**为什么的推理链** | `02-conversations.md` | ★★★★★ |
...
| 5 决策 | 重大决策、转折点、争议行为 | 决策背景与逻辑、**事后反思与承认错误**、言行一致/不一致案例 | `05-decisions.md` | ★★★★★ |
...
- 发现矛盾时保留矛盾，不要和稀泥
- **追踪立场转变**：记录此人何时改变观点，转变前后的理由

#### Agent prompt模板 v2.0

spawn subagent时，用以下结构给任务（以Agent 2对话为例）：
...
- 每条信息标注来源URL和可信度
- 区分一手（此人直接说）vs 二手（别人转述）
- 记录矛盾，不自行调和
- 标注[过程密度]：高（有完整推理链）/中（结论+部分解释）/低（只有结论）
- 标注[时间标注]：显性（直接说变）/隐晦（行为变化）/无（无变化）
- 追踪立场转变，记录转变前后的理由

信息源黑名单：不使用知乎、微信公众号、百度百科。
```

其他5个Agent按同样结构调整搜索方向和输出要求即可。
...
- **Phase 1.5 预提炼**：`python3 [skill目录]/scripts/pre_extract.py <skill目录>`
  - 自动扫描 `references/research/01-06.md`，生成结构化摘要报告
  - 统计来源数、一手/二手占比、关键发现、矛盾点、缺口维度
  - 输出时间跨度、思想转折点信号、质量门禁确认框
...
| 本人著作 | 系统性思考 | 最高 |
| **长对话/访谈中被追问"为什么"** | 即兴思维过程，推理链 | **最高** |
| **决策后反思与承认错误** | 真实推理过程 | **最高** |
...
- **过程信息严重缺失**：Agent 2或Agent 5返回素材过少时，在Phase 1.5报告中标注，降低整体置信度预期

**关键规则**：宁可生成一个诚实标注了局限的60分Skill，也不要生成一个看起来完美但实际上在编造的90分Skill。

### Phase 1.5: 预提炼与质量门禁（新增） v2.0

**目的**：在 Phase 2 提炼前，先用工具扫描6份Agent文件，生成结构化摘要，让提炼者有针对性地工作而非裸读全文。

#### Step 1: 运行预提炼工具

```
python3 scripts/pre_extract.py <skill目录>
```
...
┌─────────────────────────────────────────────────────────┐
│  来源统计                                               │
│  Agent1 著作: 8篇 │ Agent2 对话: 3段 │ Agent3 表达: 120条│
│  Agent5 决策: 2个 │ Agent4 他者: 4篇 │ Agent6 时间线: 完整│
├─────────────────────────────────────────────────────────┤
│  置信度总评                                             │
│  一手来源: 65% │ 二手: 25% │ 推测: 10%               │
├─────────────────────────────────────────────────────────┤
│  过程信息密度                                           │
│  Agent2 对话: 高（被追问"为什么"的场景≥3）              │
│  Agent5 决策: 中（缺少事后反思类素材）                   │
├─────────────────────────────────────────────────────────┤
│  矛盾点: 2处                                           │
│  · Agent1论点和Agent5案例矛盾（已标注，不调和）          │
├─────────────────────────────────────────────────────────┤
│  缺口维度: 2个                                         │
│  · 推理过程（Agent5缺反思类素材）                       │
│  · 近期动态（Agent6最新动态截止20XX年）                 │
├─────────────────────────────────────────────────────────┤
│  时间跨度: 1980-2024（44年）                           │
│  思想转折点信号: 强3处 / 中2处 / 弱1处                 │
└─────────────────────────────────────────────────────────┘
...
提炼者确认框：
□ 质量可接受 → 进入 Phase 2
□ 某维度严重缺失 → 补充采集后重跑 Phase 1.5
□ 整体质量不足 → 降低期望，聚焦核心维度
```

#### Step 2: 质量门禁

提炼者看到 Phase 1.5 报告后，判断：

| 情况 | 操作 |
|------|------|
| 质量可接受 | 进入 Phase 2 |
| Agent 2/5 严重缺料 | 补充采集该维度后重跑 Phase 1.5 |
| 整体置信度<50% | 坦诚告知用户，Phase 2 聚焦核心维度，诚实边界加大篇幅 |

**Phase 1.5检查点意义**：调研质量决定了最终Skill的上限。垃圾进垃圾出，在这里拦截比在Phase 4返工成本低得多。

---

### Phase 2: 框架提炼（Synthesis）v2.0
...
5. **标注置信度**：此人直接表述→🟢高；基于行为推断→🟡中；基于二手推测→🔴低
6. **标注时效状态**：从早期到近期均未改变→✅持续；有明确演化轨迹→🔄演化；后期已放弃→❌已放弃

#### 2.2 决策启发式提取（5-10条）

= 此人做判断时的快速规则。可表述为「如果X，则Y」，有具体案例支撑。每条标注来源可信度。
...
**重要**：如果此人的思想有明显演化，按时间分期提炼：

```
1. 识别思想转折点（基于 Phase 1.5 报告）：
   - 强信号：此人直接说"以前X现在Y"
   - 中信号：行为与主张矛盾、策略突变
   - 弱信号：外部评价变化（谨慎采信）
...
### Phase 3: Skill构建 v2.0

将Phase 2提炼结果组装为可运行的SKILL.md。

#### Step 1: 读取模板
读取 `references/skill-template.md` 获取标准结构 v2.0。模板定义了目标Skill的完整骨架，包含新的标注体系（置信度/时效状态/矛盾显性化/结构化诚实边界）。

#### Step 2: 填充内容
按模板结构，将Phase 2的提炼结果逐section填入：

| 模板Section | 填充来源 | v2.0新增字段 |
|------------|---------|------------|
| frontmatter description | 来源数量+模型数量+触发词 | 版本标注v2.0 |
| 角色扮演规则 | 直接使用模板默认规则，不需要改 | - |
| **回答工作流（Agentic Protocol）** | **根据心智模型自动推导，详见下方生成指引** | - |
| 身份卡 | 时间线(06) + 著作(01) → 用此人语气写50字自我介绍 | - |
| 心智模型 | Phase 2.1 提取结果 | **置信度🟢/🟡/🔴** + **时效状态✅/🔄/❌** |
| 决策启发式 | Phase 2.2 提取结果 | **来源可信度标注** |
| 表达DNA | Phase 2.3 分析结果 → 转为角色扮演时的风格规则 | - |
| 时间线 | Agent 6 调研结果，精简为关键节点表格 | **思想转折点表格** |
| 价值观与反模式 | Phase 2.4 结果 | - |
| 内在张力 | 矛盾保留 | **矛盾显性化，多解释并列** |
| 智识谱系 | Phase 2.5 结果 | - |
| 诚实边界 | Phase 2.6 + 调研时间 | **结构化分条，每条标置信度** |
| 调研来源 | 6个Agent的引用汇总，分一手/二手 | **🟢/🟡标注** |
...
#### Step 3: 质量自检
构建完成后，读取 `references/extraction-framework.md` 末尾的「质量自检清单」，逐项检查。新增v2.0检查项：
- [ ] 每个心智模型都有置信度标注（🟢/🟡/🔴）
- [ ] 演化类模型有变化轨迹说明
- [ ] 内在矛盾有多解释并列，未强行调和
- [ ] 诚实边界有结构化分条，每条标注置信度
- [ ] 一手来源占比>50%（Phase 1.5报告可查）

不通过的项标注出来，回到对应Phase修复。
...
- 不应该斩钉截铁
- 注意：如果相关模型的置信度是🔴低，边缘测试结果的可信度相应降低
...
| 每个模型的置信度 | 明确标注🟢/🟡/🔴 | 无标注或标注混乱 |
| 每个模型的局限性 | 明确写出失效条件 | 只写优点 |
| 表达DNA辨识度 | 读100字能认出是谁 | 像通用ChatGPT |
| 诚实边界 | 至少3条具体局限，含置信度 | 只有「不能替代本人」 |
| 内在张力 | 至少2对矛盾，多解释并列 | 观点高度一致（太假） |
| 一手来源占比 | >50% | 主要依赖二手转述 |
| 时效状态标注 | 每个模型标注✅/🔄/❌ | 缺失或全部标注✅ |
...
1. 读取现有SKILL.md，从「诚实边界」section中找到「调研截止: [日期]」，标注距今多久
2. 只启动Agent 2（最新对话）+ Agent 5（最新决策）+ Agent 6（时间线更新）
3. 对比新信息与现有内容：
   - 新信息强化现有模型 → 补充案例，维持置信度
   - 新信息与现有模型矛盾 → 标注变化，更新模型状态（✅持续→🔄演化）
   - 出现新的思维模式 → 考虑增加新模型
   - 某模型后期已放弃 → 更新为❌已放弃
4. 更新SKILL.md中的「最新动态」section、思想转折点和调研时间
5. 不重写整个Skill，只增量更新
...
### 活人 vs 历史人物
- **活人**：注意时效性，标注截止日期，建议定期更新。近期观点比早期观点权重更高
- **历史人物**：材料更稳定但可能有传记偏差，多源交叉验证。时效状态更多是✅持续或❌已放弃
...
2. 心智模型减至2-3个，每个都标注🔴推测，诚实边界加大篇幅
3. 诚实边界中明确列出「哪些维度信息严重不足」
...
**v2.0更新**：通过Phase 1.5预提炼、三层置信度标注、时间维度分期提炼，我们让这个镜子更诚实地呈现——哪些是此人真正说过的话，哪些是推测，哪些可能已经过时。
